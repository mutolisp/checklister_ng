    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onCancel}>
      <View className="flex-1 bg-white" style={{ paddingTop: insets.top, paddingBottom: insets.bottom }}>
        <View className="flex-row items-center justify-between border-b border-gray-200 px-4 py-3">
          ...
          <Text className="text-base font-semibold text-gray-900">{isEmpty ? '空 session' : '結束記錄'}</Text>
          {isEmpty ? (
            <Pressable onPress={onDeleteEmpty} hitSlop={8}>
              <Text className="text-base font-semibold text-red-600">刪除</Text>
            </Pressable>
          ) : (
            <Pressable onPress={() => onConfirm({ name, project_id: projectId, notes })} hitSlop={8}>
              <Text className="text-base font-semibold text-red-600">結束</Text>
            </Pressable>
          )}
        </View>
        <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView className="flex-1">
            {isEmpty ? (
              <View className="border-b border-gray-100 bg-amber-50 px-4 py-4">
                <Text className="text-sm font-medium text-amber-900">這個 session 還沒有任何紀錄。</Text>
                <Text className="mt-1 text-sm text-amber-800">
                  按右上「刪除」直接清掉，或按取消回去繼續加入物種。
                </Text>
              </View>
            ) : (
              <View className="border-b border-gray-100 bg-blue-50 px-4 py-4">
            ...
            <Field label="名稱">
              <TextInput
                ...
                className="border border-gray-300 rounded px-3 py-2 text-base text-gray-900"
                placeholder="session 名稱"
      ...
      <Modal visible={showProjectPicker} transparent animationType="slide" onRequestClose={() => setShowProjectPicker(false)}>
        <Pressable onPress={() => setShowProjectPicker(false)} className="flex-1 bg-black/40">
          <Pressable className="mt-auto rounded-t-2xl bg-white" style={{ paddingBottom: insets.bottom + 8 }}>
            ...
            <ScrollView className="max-h-96">
              {projects.map((p) => {
                ...
                return (
                  ...
                  >
                    <Text className={`flex-1 text-base ${active ? 'font-semibold text-blue-700' : 'text-gray-900'} ${p.id === 0 ? 'italic text-gray-500' : ''}`}>
                      {p.name}
                    </Text>
