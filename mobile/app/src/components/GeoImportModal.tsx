    ...
    <Modal visible={visible} animationType="slide" onRequestClose={handleClose}>
      ...
      >
        <View className="flex-row items-center justify-between border-b border-gray-200 px-4 py-3">
          ...
          <Text className="text-base font-semibold text-gray-900">匯入地理檔案</Text>
          <Pressable onPress={handleCommit} hitSlop={8} disabled={imported.length === 0}>
            <Text
              className={`text-base font-semibold ${imported.length === 0 ? 'text-gray-300' : 'text-blue-600'}`}
            >
              建立 {imported.length || ''}
            </Text>
          </Pressable>
            ...
            <>
                  ...
                  {projects.map((p) => {
                    ...
                    return (
                      <Pressable
                        ...
                        className={`rounded-full border px-3 py-1.5 ${active ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-white'}`}
                      >
                        <Text
                          className={`text-xs ${active ? 'font-semibold text-blue-700' : 'text-gray-700'} ${p.id === 0 ? 'italic' : ''}`}
                        >
                          {p.name}
                        </Text>
                      </Pressable>
              ...
              <View className="px-4 py-3">
                ...
                {imported.slice(0, 20).map((item, i) => (
                  <View key={i} className="border-b border-gray-100 py-2">
                    <Text className="text-sm text-gray-900">
                      {item.name || `${namePrefix || '匯入'} #${i + 1}`}
                    </Text>
                    <Text className="text-xs text-gray-500">{item.geometry.type}</Text>
                  </View>
                ))}
                {imported.length > 20 ? (
              ...
              <View className="px-4 py-3">
                <Pressable
                  onPress={handlePickFile}
                  className="flex-row items-center self-start rounded-full bg-gray-100 px-3 py-1.5 active:bg-gray-200"
                >
                  <Ionicons name="folder-open-outline" size={14} color="#374151" />
                  <Text className="ml-1 text-xs font-medium text-gray-700">換另一個檔案</Text>
                </Pressable>
              </View>
            </>
          )}
        ...
        {currentProject ? (
          <View className="border-t border-gray-100 px-4 py-2">
            <Text className="text-xs text-gray-500">
              目標專案：<Text className="font-medium text-gray-700">{currentProject.name}</Text>
            </Text>
          </View>
        ) : null}
      </View>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <View className="border-b border-gray-100 px-4 py-3">
      <Text className="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500">{label}</Text>
      {children}
    </View>
  );
}
