...
const ITEMS: MenuItem[] = [
  ...
  { icon: 'search-outline', label: '物種查詢', description: '不歸屬任何 session 的查詢', href: '/lookup' },
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      <View className="border-b border-gray-200 bg-white px-4 py-6">
        <Text className="text-2xl font-bold text-gray-900">選單</Text>
      </View>
      <View className="mt-2 bg-white">
        {ITEMS.map((item, idx) => (
          <Pressable
            ...
            className={`flex-row items-center border-b border-gray-100 px-4 py-4 ${item.disabled ? 'opacity-40' : 'active:bg-gray-100'} ${idx === ITEMS.length - 1 ? 'border-b-0' : ''}`}
          >
            <Ionicons name={item.icon} size={24} color="#4b5563" />
            <View className="ml-3 flex-1">
              <Text className="text-base text-gray-900">{item.label}</Text>
              {item.description ? <Text className="text-xs text-gray-500">{item.description}</Text> : null}
            </View>
