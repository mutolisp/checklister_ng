...
`⋯` 選單加入第四項：**統計 / 移動到專案 / 合併 / 刪除**。列表與卡片兩種檢視共用同一條選取工具列，所以兩邊都有。

### 放在選單最後、標為 destructive
與左滑動作的慣例一致（破壞性動作擺最後、最靠近取消），拇指不會因為習慣位置而誤觸。`showActionSheet` 的 `destructive` 在 iOS 走原生紅字、Android 走自家 Modal 的紅字，兩邊都有視覺警示。

### 確認對話框同時報「幾筆記錄」與「幾筆物種紀錄」
```
刪除 3 筆調查記錄？
這 3 筆調查記錄與其下 412 筆物種紀錄將全部移除，無法復原。
```
只說「刪除 3 筆」會嚴重低估實際丟掉的東西 —— 這是選取工具列上唯一無法復原的動作。數字取自 `RecordItem.recordCount` 加總，不另外查 DB。

### 實作
`handleDeleteSelection`：`withTransaction` 內依 kind 分派 `deleteSession` / `deletePlotSurvey` / `deleteCollectionTrip`（與單筆刪除同一組 helper），然後 `reload()` + `selectClear()` + toast。

**不需要另外刷新 active store**：records 分頁的 `reload()` 本來就會呼叫 `refreshActive()` / `refreshActivePlot()`，而批次刪除很可能刪到正在進行的那筆。

照片與錄音檔**不會**被刪除（沿用既有的刪除行為：`deleteSession` 等只刪列）。

### i18n
新增 `records.deleteSelectedTitle` / `deleteSelectedMsg` / `deletedCount`（7 個完整語系，複數類別齊全）。選單標籤直接用既有的 `common.delete`。

> 本來想重用 `favorites.deleteSelected`（「刪除所選」），查了各語系後放棄 —— **日文寫的是「選択した種を削除」**，那是針對物種的字串，用在調查記錄上會是錯的。

### 驗證狀態
- [x] tsc / lint 0 error / check:i18n（1471 鍵）・dock・kav・names・diversity・roundtrip・vegmatrix・report・inat
- [ ] **實機**：長按進多選 → 選 2 筆以上 → `⋯` → 刪除 → 對話框數字正確 → 確認後列表更新、toast 顯示筆數
- [ ] **實機**：取消對話框後什麼都沒被刪、選取狀態仍在
- [ ] **實機**：刪掉「正在進行」的那筆之後，頂端的 ActiveSessionBar 跟著消失（不是留一個幽靈）

### 後續修正（使用者標注的截圖）

**1. 返回鍵的 `<` 沒有在圓底裡置中。**
chevron 本來就不對稱於自己的字框：尖端在一邊、兩臂朝另一邊張開，幾何置中會看起來偏向尖端。裸圖示時看不出來，一放進圓底就明顯 —— 這正是 `goBack.tsx` 原本註解提到的 bearing 問題，只是以前沒有底可以對照。

`HeaderIconButton` 加一張 `OPTICAL_NUDGE_X` 表（`chevron-back: +2`、`chevron-forward: −2`），只列真的需要的字符。

> 第一版我寫成 `marginLeft`，**那是錯的**：按鈕會把子元件置中，margin 同時撐寬了子元件，實際位移只有設定值的一半 —— 註解說 2px、實際只有 1px。改用 `transform: translateX`，不影響版面配置，位移才等於寫下去的數字。

**2. 右側兩顆圓底黏成一塊。**
`gap-2`（8px）在兩顆 36pt 圓底之間讀起來像一個膠囊而不是兩顆按鈕。三個畫面的 headerRight 都改 `gap-3`（12px）。

> 專案詳細頁最多 4 顆（版面切換／報告／編輯／匯出），`4×36 + 3×12 = 180pt`，在 375pt 寬的機型上標題只剩約 140pt，較長的專案名會被截斷。若嫌擠，那個畫面的按鈕數本身才是該減的東西。

- [ ] **實機**：返回鍵的 `<` 在圓底裡目視置中（2px 是視覺判斷，實機看了再微調）
- [ ] **實機**：樣區／名錄 header 右側兩顆分得開；專案頁 4 顆時標題還讀得出來

---

## 2026-09-16 — 導覽列再瘦身：專案編輯、篩選群組、常用名錄改愛心

### 1. 專案頁的「編輯」移到標題上
`app/project/[id].tsx` 的 headerRight 從 4 顆減為 3 顆（版面／報告／匯出），編輯改成**點標題**，鉛筆小圖示跟在名稱後面 —— 與採集改名完全同一個 pattern（`app/collection/[id].tsx` 的 `headerTitle`）。

除了騰出位置，這個做法本身也比較對：編輯的 affordance 貼在它要編輯的東西上，而不是飄在一排按鈕裡。「未分類」（id 0）不是真的專案、不能改名，所以那個情況下標題不可點、鉛筆也不畫。

### 2. 篩選 / 檢視 / 匯入 收進「⋯」
記錄分頁標題列從「標題 + 橫向捲動的 3 個 chip + 常用名錄藥丸 + 匯入鈕」變成**標題 + 3 顆等高藥丸（版面切換 / 常用名錄 / ⋯）**。
...
各多一下點擊，換回來的是 icon-only chip 當初丟掉的東西 —— 漏斗圖示永遠說不出「現在篩的是什麼」。

因為三顆藥丸都沒有文字，header 寬度不再隨 UI 語言變動，原本用來吸收這種成長的**橫向 ScrollView 也一併拿掉**，標題改成 `flex-1` 吃掉剩下的寬度並截斷。

連帶清掉：`ScrollView` import、以及只為篩選／檢視存在的 `DropdownChip` 元件（本次改動造成的 orphan）。

### 3. 常用名錄：星星 → 愛心，去掉文字
三處入口（記錄分頁標題列、多選工具列的「加入常用名錄」、物種詳細頁的收藏切換）改用 `heart` / `heart-outline`，維持既有的琥珀色。標題列那顆改成純圖示藥丸。

**這不只是換個造型**：記錄現在自己有星號（加星號），常用名錄也用星號的話，兩個不同功能共用一個符號，野外操作遲早按錯。改完之後語意乾淨了 —— **星＝預設／置頂，心＝常用名錄**。

刻意沒動的星號：`app/surveyors.tsx`（預設調查者）與 `app/favorites.tsx`（預設名錄資料夾），那兩個講的是「預設」不是「收藏」。

> 愛心目前仍是琥珀色（沿用既有的常用名錄配色）。若想要更常規的紅／玫瑰色，藥丸底色與另外兩處也要一起調。

### 驗證狀態
- [x] tsc / lint 0 error / check:i18n・dock・kav・names・diversity・roundtrip・vegmatrix・report・inat
- [x] grep 確認 app 內殘留的 `star` 只剩「預設」語意的三處
- [ ] **實機**：專案頁點標題會開編輯；「未分類」點不動也沒有鉛筆
- [ ] **實機**：⋯ → 篩選／檢視 的兩層 sheet 在 iOS 正常跳出（中間有 250ms 等待）
- [ ] **實機**：記錄分頁標題列三顆藥丸等高、法文等長標題下不擠壓按鈕

---

## 2026-09-16 — 常用名錄：長按多選、標題改名、返回鍵統一

### 1. 長按多選，取消打勾按鈕
名錄清單與物種清單都改成**長按進多選**（含 Medium haptics、長按那一筆直接被選起來），與記錄分頁完全同一個手勢。兩個「☑」進入鈕移除。

長按預設名錄時只進入模式、不選它自己 —— 預設名錄不能刪，勾起來只會讓「已選 1／刪除」互相矛盾。

### 2. 物種的長按選單搬到左滑
長按原本開的是「加到記錄／移動到／移除」。手勢被多選佔用後，這三項移到左滑動作列（`加到記錄` emerald、`移動到` blue、`移除` red 且在最後），也就是記錄分頁的模型：**長按＝選取，左滑＝單筆動作**。「移動到」只在有第二個名錄時出現。

### 3. 重新命名移到標題
進入名錄後，標題旁出現鉛筆，點標題即改名 —— 與採集改名、專案改名同一個 pattern。原本它藏在資料夾的長按選單裡，而長按現在另有意義。

### 4. 返回鍵：修好 + 統一
**`app/_layout.tsx` 給 favorites 註冊時沒有 `headerLeft`**，所以它一直用系統原生返回鍵：藍色方角（與全 app 新的青綠圓底不一致），而且在名錄**內部**按下去是直接離開整個常用名錄功能，不是回到名錄清單 —— 使用者說的「往左 symbol 失效」。

改法：兩個層級都自帶 `headerLeft`，用共用的 `HeaderIconButton`。
- 名錄清單 → `goBackOrHome()`（含無 history 的 fallback）
- 名錄內部 → `setFolderId(null)`，回上一層

header 既然負起責任，**內容區那條「‹ 所有名錄」也一併移除** —— 它本來就是在補系統返回鍵的洞。

### 還沒動
畫面內的 `匯入` / `排序` / `新增名錄` 藥丸仍是既有的藍底樣式。那些有文字標籤、位在內容區而非導覽列，與 header 的青綠圖示鈕不是同一類控制項；要不要一起換成中性藥丸可以再決定。

### 驗證狀態
- [x] tsc / lint 0 error（清掉 `ActionSheetOption` 這個本次造成的 orphan import）
- [x] check:i18n・dock・kav・names・diversity・roundtrip・vegmatrix・report・inat 全過
- [ ] **實機**：名錄清單長按 → 進多選並選中該筆；長按預設名錄 → 進模式但未選中
- [ ] **實機**：名錄內部按返回 → 回到名錄清單（不是離開常用名錄）；清單按返回 → 回上一頁
- [ ] **實機**：點名錄標題可改名，鉛筆看得到
- [ ] **實機**：物種左滑有三個動作，且「移動到」在只有一個名錄時不出現

### 後續：三顆藥丸改中性，並抽成共用元件

`匯入` / `排序` / `新增名錄` 原本是三種樣式（前兩者藍底藍字、排序灰底灰字），現在統一為中性藥丸：白底 + `border-gray-300` + 灰圖示灰字，與記錄分頁標題列的藥丸同一個外觀。

順手抽成 `src/components/PillButton.tsx`，因為算上記錄分頁那三顆，全 app 已經有 6 個地方在畫同一顆藥丸（其中記錄分頁的版本還是上一輪剛寫的區域元件 `PillIconButton`）。API 是 `icon` + 選填 `label`（省略即純圖示）+ `tone`。

**這樣上色才有意義**：以前每個工具列都像「有一顆重要按鈕加幾顆附屬品」，而藍色並不代表什麼。現在中性是預設，`tone="amber"` 只留給常用名錄。

轉換的 6 處：favorites 的 匯入／排序／新增名錄，記錄分頁的 版面切換／常用名錄（amber 愛心）／⋯。區域元件 `PillIconButton` 隨之刪除。
